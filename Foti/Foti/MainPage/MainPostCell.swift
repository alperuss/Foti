...
class MainPostCell : UICollectionViewCell {
    ...
    var post : Post? {
        didSet{
            ...
            attrPostMessage()
            if post?.liked == true{
                btnLike.setImage(UIImage(named: "Like_Selected")?.withRenderingMode(.alwaysOriginal), for: .normal)
            } else {
                btnLike.setImage(UIImage(named: "Like_Not_Selected")?.withRenderingMode(.alwaysOriginal), for: .normal)
            }
    ...
    @objc fileprivate func btnCommentPressed(){
        guard let post = self.post else {return}
        delegate?.pressedComment(post: post)
    }
