...
class CameraController : UIViewController,UIViewControllerTransitioningDelegate {
    let btnBack : UIButton = {
        ...
        btn.setImage(UIImage(named: "Arrow_Left")?.withRenderingMode(.alwaysOriginal), for: .normal)
    ...
    fileprivate func editView() {
        ...
        btnBack.anchor(top: view.topAnchor , bottom: nil , leading: view.leadingAnchor, trailing: nil, paddingTop: 25, paddingBottom: 0, paddingLeft: 20, paddingRight: 0, width: 50, height: 50)
        btnTakePhoto.centerXAnchor.constraint(equalTo: view.centerXAnchor).isActive = true
    }
    ...
    let animationPresentation = AnimationPresentation()
    let animationDismissPresentation = AnimationDismissPresentation()
    
    func animationController(forPresented presented: UIViewController, presenting: UIViewController, source: UIViewController) -> UIViewControllerAnimatedTransitioning? {
        return animationPresentation 
    }
    func animationController(forDismissed dismissed: UIViewController) -> UIViewControllerAnimatedTransitioning? {
        return animationDismissPresentation
    }
}
